package leetcode.editor.en.Q292;
import java.util.*;
...
class Solution {
    ...
    public boolean canWinNim(int n) {
        cache = new Boolean[5];
        return canWinNim((n % 4), 0);
    }

    public boolean canWinNim(int n, int player) {
        if (n <= 3) {
            return n > 0;
        }
        ...
        cache[n] = !(canWinNim(n - 1, (player + 1) % 2) &&
                canWinNim(n - 2, (player + 1) % 2) &&
                canWinNim(n - 3, (player + 1) % 2));
        return cache[n];
    }
}
...
public class NimGame extends Solution {}