package leetcode.editor.en.Q2363;
...
class Solution {
    public List<List<Integer>> mergeSimilarItems(int[][] items1, int[][] items2) {
        TreeMap<Integer, Integer> itemsSum = new TreeMap<>();

        for (int[] item : items1) {
            itemsSum.put(item[0], itemsSum.getOrDefault(item[0], 0) + item[1]);
        }
        for (int[] item : items2) {
            itemsSum.put(item[0], itemsSum.getOrDefault(item[0], 0) + item[1]);
        }
        ...
        for (Map.Entry<Integer, Integer> kv : itemsSum.entrySet()) {
            res.add(List.of(kv.getKey(), kv.getValue()));
        }
        return res;
    }
}
...
public class MergeSimilarItems extends Solution {
}