package leetcode.editor.en.Q2562;
...
class Solution {
    ...
    public long findTheArrayConcVal(int[] nums) {
        ...
        while (left <= right) {
            if (left != right) {
                res.add(new BigInteger(String.valueOf(nums[left]) + nums[right]));
            } else {
                res.add(new BigInteger(String.valueOf(nums[left])));
            }
            left++;
            right--;
        }
...
public class FindTheArrayConcatenationValue extends Solution {
}