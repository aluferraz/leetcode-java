package leetcode.editor.en.Q486;
...
class Solution {
    ...
    public boolean PredictTheWinner(int[] nums) {
        ...
        return play(0, nums.length - 1, nums) >= 0;
    ...
    private int play(int left, int right, int[] nums) {
        ...
        int pathOne = nums[left] - play(left + 1, right, nums);
        int pathTwo = nums[right] - play(left, right - 1, nums);
        cache[left][right] = Math.max(pathOne, pathTwo);
...
public class PredictTheWinner extends Solution {
}