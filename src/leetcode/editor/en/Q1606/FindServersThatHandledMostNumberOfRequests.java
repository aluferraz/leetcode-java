package leetcode.editor.en.Q1606;
    ...
    public List<Integer> busiestServers(int k, int[] arrival, int[] load) {
        ...
        for (int i = 0; i < arrival.length; ++i) {
            ...
            while (!busy.isEmpty() && busy.peek().getKey() <= start) {
                Pair<Integer, Integer> curr = busy.poll();
        ...
        PriorityQueue<int[]> busiest = new PriorityQueue<>((a, b) -> {
            //int aServer = a[0];
            int aRequestsHandled = a[1];

            // int bServer = b[0];
            int bRequestsHandled = b[1];
            return Integer.compare(bRequestsHandled, aRequestsHandled);
        });
        for (int i = 0; i < count.length; i++) {
            busiest.add(new int[]{i, count[i]});
        }

        int overallBusiest = busiest.peek()[1];

        List<Integer> res = new ArrayList<>();

        while (!busiest.isEmpty() && busiest.peek()[1] == overallBusiest) {
            res.add(busiest.poll()[0]);
        }
        return res;
    }
}
...
public class FindServersThatHandledMostNumberOfRequests extends Solution {
}