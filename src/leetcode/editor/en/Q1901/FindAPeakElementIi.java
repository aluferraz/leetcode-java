package leetcode.editor.en.Q1901;
...
class Solution {
    ...
    //The log N is because no adjacents cells are equal and we only evaluate the bigger-than cells
    public int[] findPeakGrid(int[][] mat) {
        return findPeakGridHelper(mat, 0, 0);
    }

    private int[] findPeakGridHelper(int[][] mat, int i, int j) {
        boolean isPeak = true;
        int[] peak;
        if (isValidIdx(i - 1, j, mat) && mat[i - 1][j] >= mat[i][j]) {
            isPeak = false;
            peak = findPeakGridHelper(mat, i - 1, j);
            if (peak[0] != -1) return peak;
        }
        ...
        if (isValidIdx(i, j - 1, mat) && mat[i][j - 1] >= mat[i][j]) {
            isPeak = false;
            peak = findPeakGridHelper(mat, i, j - 1);
            if (peak[0] != -1) return peak;
        }
...
public class FindAPeakElementIi extends Solution {
}