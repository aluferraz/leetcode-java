package leetcode.editor.en.Q1700;
...
class Solution {
    public int countStudents(int[] students, int[] sandwiches) {
        LinkedList<int[]> studentsQ = new LinkedList<>();
        for (int i = students.length - 1; i >= 0; i--) {
            studentsQ.add(new int[]{students[i], -1});
        }
        int sandwichIdx = 0;
        while (!studentsQ.isEmpty()) {
            int[] info = studentsQ.peek();
            int pref = info[0];
            int lastTry = info[1];
            if (sandwichIdx == lastTry) break;
            studentsQ.poll();
            if (sandwiches[sandwichIdx] == pref) {
                sandwichIdx++;
                continue;
            }
            info[1] = sandwichIdx;
...
public class NumberOfStudentsUnableToEatLunch extends Solution {
}