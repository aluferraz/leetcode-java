...
class Solution {
    public int myAtoi(String s) {
        try {
            ...
            String number = s.substring(left, pos);

            BigInteger res = new BigInteger(number);


            BigInteger minimum = new BigInteger(String.valueOf(Integer.MIN_VALUE));
            BigInteger maximum = new BigInteger(String.valueOf(Integer.MAX_VALUE));
            if (res.compareTo(minimum) < 0) {
                return Integer.MIN_VALUE;
            }
            if (res.compareTo(maximum) > 0) {
...
public class StringToIntegerAtoi extends Solution {
}