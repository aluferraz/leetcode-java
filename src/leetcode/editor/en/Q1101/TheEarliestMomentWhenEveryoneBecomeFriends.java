package leetcode.editor.en.Q1101;
...
class Solution {
    public int earliestAcq(int[][] logs, int n) {
        ...
        for (int[] edge : logs) {
            int from = edge[1];
            int to = edge[2];
            components -= union(from, to, parents);
            if (components == 1) return edge[0];
        }
        return -1;
    }
    ...
    private int union(int i, int j, int[] parents) {
        i = find(i, parents);
        j = find(j, parents);
        if (i == j) return 0;
...
public class TheEarliestMomentWhenEveryoneBecomeFriends extends Solution {
}