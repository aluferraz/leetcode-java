package leetcode.editor.en.Q1061;
...
class Solution {
    public String smallestEquivalentString(String s1, String s2, String baseStr) {
        ...
        for (int i = 0; i < N; i++) {
            char c1 = s1.charAt(i);
            char c2 = s2.charAt(i);
            int p1 = getParent(c1 - 'a', alpha);
            int p2 = getParent(c2 - 'a', alpha);
            if (p1 != p2) {
                join(p1, p2, alpha);
            }
        }
        ...
        for (int i = 0; i < M; i++) {
            ...
            int parent = getParent(c - 'a', alpha);
            if (parent == -1) continue; // Same letter
            if (parent < (c - 'a')) res[i] = (char) ('a' + parent);
        }
        return String.valueOf(res);

    }

    private void join(int i, int j, int[] array) {
        //We want to point to the smallest letter
        if (i < j) {
            array[j] = i;
            return;
        }
    ...
    private int getParent(int i, int[] array) {
        while (array[i] >= 0) {
            i = array[i];
        }
        return i;
...
public class LexicographicallySmallestEquivalentString extends Solution {
}