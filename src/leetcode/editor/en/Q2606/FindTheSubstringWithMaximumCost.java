package leetcode.editor.en.Q2606;
...
class Solution {
    ...
    public int maximumCostSubstring(String s, String chars, int[] vals) {
        ...
        for (int i = 0; i < chars.length(); i++) {
            values[chars.charAt(i) - 'a' + 1] = vals[i];
        }
        maxCostHelper(0, s, values);
        return res;
    }

    private int maxCostHelper(int i, String s, int[] values) {
        if (i == s.length()) return 0;
        if (cache[i] != null) return cache[i];

        int cost = values[s.charAt(i) - 'a' + 1];

        int useAhead = cost + maxCostHelper(i + 1, s, values);
        int doNotUseAhead = cost;


        cache[i] = Math.max(useAhead, doNotUseAhead);
...
public class FindTheSubstringWithMaximumCost extends Solution {
}