package leetcode.editor.en.Q2654;
import java.util.*;
import javafx.util.Pair;

//You are given a 0-indexed array nums consisiting of positive integers. You 
//can do the following operation on the array any number of times: 
//
// 
// Select an index i such that 0 <= i < n - 1 and replace either of nums[i] or 
//nums[i+1] with their gcd value. 
// 
//
// Return the minimum number of operations to make all elements of nums equal 
//to 1. If it is impossible, return -1. 
//
// The gcd of two integers is the greatest common divisor of the two integers. 
//
// 
// Example 1: 
//
// 
//Input: nums = [2,6,3,4]
//Output: 4
//Explanation: We can do the following operations:
//- Choose index i = 2 and replace nums[2] with gcd(3,4) = 1. Now we have nums =
// [2,6,1,4].
//- Choose index i = 1 and replace nums[1] with gcd(6,1) = 1. Now we have nums =
// [2,1,1,4].
//- Choose index i = 0 and replace nums[0] with gcd(2,1) = 1. Now we have nums =
// [1,1,1,4].
//- Choose index i = 2 and replace nums[3] with gcd(1,4) = 1. Now we have nums =
// [1,1,1,1].
// 
//
// Example 2: 
//
// 
//Input: nums = [2,10,6,14]
//Output: -1
//Explanation: It can be shown that it is impossible to make all the elements 
//equal to 1.
// 
//
// 
// Constraints: 
//
// 
// 2 <= nums.length <= 50 
// 1 <= nums[i] <= 10⁶ 
// 
//
// 👍 160 👎 7


//leetcode submit region begin(Prohibit modification and deletion)
import java.math.BigInteger;

class Solution {


    public int minOperations(int[] nums) {
        int alreadyConverted = 0;
        for (int i = 0; i < nums.length; i++) {
            int num = nums[i];
            if (num == 1) {
                alreadyConverted++;
            }
        }
        if (alreadyConverted > 0) {
            return nums.length - alreadyConverted;
        }
        int opsToCreateOne = 100000000;
        for (int i = 0; i < nums.length; i++) {
            opsToCreateOne = Math.min(minOperations(i, nums), opsToCreateOne);
        }
        if (opsToCreateOne >= 100000000) return -1;
        return nums.length + opsToCreateOne - 1;

    }


    private int minOperations(int i, int[] nums) {
        if (i >= nums.length - 1 || i < 0) {
            return 100000000;
        }

        int num = nums[i];
        int next = nums[i + 1];
        int gcd = new BigInteger(String.valueOf(num)).gcd(new BigInteger(String.valueOf(next))).intValue();
        if (gcd == 1) {
            return 1;
        }
        if (next == gcd && num == gcd) {
            return 100000000;
        }
        // Try change i
        int changeHere = 100000000;
        int changeNext = 100000000;
        if (num != gcd) {
            nums[i] = gcd;
            changeHere = 1 + minOperations(i - 1, nums);
            nums[i] = num;
        }
        // Try change i + 1;
        if (next != gcd) {
            nums[i + 1] = gcd;
            changeNext = 1 + minOperations(i + 1, nums);
            nums[i + 1] = next;
        }

        return Math.min(changeHere, changeNext);

    }

}
//leetcode submit region end(Prohibit modification and deletion)



public class MinimumNumberOfOperationsToMakeAllArrayElementsEqualTo1 extends Solution {}