package leetcode.editor.en.Q1519;
...
class Solution {
    ...
    private void dfs(int i,
        ...
        int index = c - 'a';
        int countBefore = counter[index];
        counter[index]++;
        if (adjList.containsKey(i)) {
            HashSet<Integer> edges = adjList.get(i);
            for (int edge : edges) {
                if (edge != parent) {
                    dfs(edge,
                            i,
        ...
        int countHere = counter[index] - countBefore;
        res[i] = countHere;
    }
...
public class NumberOfNodesInTheSubTreeWithTheSameLabel extends Solution {
}