package leetcode.editor.en.Q721;
...
class Solution {
    ...
    public List<List<String>> accountsMerge(List<List<String>> accounts) {
        HashMap<String, ArrayList<Integer>> emailMap = new HashMap<>();

        for (int i = 0; i < accounts.size(); i++) {
            List<String> accountsList = accounts.get(i);
            int len = accountsList.size();
            for (int j = 1; j < len; j++) {
                String email = accountsList.get(j);
                emailMap.computeIfAbsent(email, (v) -> new ArrayList<>()).add(i);
            }
        }
        ...
        for (int i = 0; i < accounts.size(); i++) {
            ...
            List<String> accountsList = accounts.get(i);
            String name = accountsList.get(0);
            TreeSet<String> emailList = new TreeSet<>();
            getEmails(i, accounts, emailMap, visited, emailList);
            List<String> accountEmails = new ArrayList<>();
            accountEmails.add(name);
            accountEmails.addAll(emailList);
            res.add(accountEmails);
        }
        return res;
    }

    private void getEmails(int i,
                           List<List<String>> accounts,
                           HashMap<String, ArrayList<Integer>> emailMap,
        ...
        for (int j = 1; j < len; j++) {
            String email = accountsList.get(j);
            result.add(email);
            for (int k : emailMap.get(email)) {
                if (k != i) {
                    getEmails(k, accounts, emailMap, visited, result);
                }
            }
        }
    }
...
public class AccountsMerge extends Solution {
}