package leetcode.editor.en.Q2243;
...
class Solution {
    public String digitSum(String s, int k) {
        ...
        while (left < s.length()) {
            res.append(sumGroup(s, left, right));
            left += k;
            right = left + k;
        }
    ...
    private int sumGroup(String s, int left, int right) {
        int res = 0;
        for (int i = left; i < Math.min(right, s.length()); i++) {
...
public class CalculateDigitSumOfAString extends Solution {
}