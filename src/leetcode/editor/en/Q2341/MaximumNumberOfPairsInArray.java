package leetcode.editor.en.Q2341;
...
class Solution {
    public int[] numberOfPairs(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            int stateIdx = Math.abs(nums[i]);
            nums[i] = state[stateIdx];
            state[stateIdx] *= -1;
        }
        int leftovers = Arrays.stream(nums).sum();

        return new int[]{
                (nums.length - leftovers) / 2,
                leftovers};
    }
}
...
public class MaximumNumberOfPairsInArray extends Solution {
}