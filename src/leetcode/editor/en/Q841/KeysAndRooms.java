package leetcode.editor.en.Q841;
...
class Solution {
    boolean[] roomsToVisit;

    public boolean canVisitAllRooms(List<List<Integer>> rooms) {
        roomsToVisit = new boolean[rooms.size()];
        roomsToVisit[0] = true;
        return visitRooms(0, rooms) == roomsToVisit.length;

    }

    private int visitRooms(int currentRoom, List<List<Integer>> rooms) {
        List<Integer> keys = rooms.get(currentRoom);
        int unlocked = 1;
        for (int nextRoom : keys) {
            if (!roomsToVisit[nextRoom]) {
                roomsToVisit[nextRoom] = true;
                unlocked += visitRooms(nextRoom, rooms);
            }
        }
        return unlocked;
    }
...
public class KeysAndRooms extends Solution {
}