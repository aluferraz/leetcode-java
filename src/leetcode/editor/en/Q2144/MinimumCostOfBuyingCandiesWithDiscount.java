package leetcode.editor.en.Q2144;
...
class Solution {
    public int minimumCost(int[] cost) {
        PriorityQueue<Integer> maxCost = new PriorityQueue<>(Comparator.reverseOrder());
        Queue<Integer> toBuy = new LinkedList<>();
        Stack<Integer> bought = new Stack<>();
        ...
        while (!maxCost.isEmpty()) {
            if (bought.size() == 2) {
                int free = Math.min(bought.pop(), bought.pop());
                while (!maxCost.isEmpty() && maxCost.peek() > free) {
                    toBuy.add(maxCost.poll());
                }
                if (!maxCost.isEmpty()) {
                    maxCost.poll();
                }
                maxCost.addAll(toBuy);
                toBuy.clear();
            }
            if (!maxCost.isEmpty()) {
                int next = maxCost.poll();
                res += next;
                bought.add(next);
            }
        }
...
public class MinimumCostOfBuyingCandiesWithDiscount extends Solution {
}