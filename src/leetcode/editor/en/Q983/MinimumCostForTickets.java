package leetcode.editor.en.Q983;
...
//            daysSet.add(day);
        ...
        return minCostHelper(daysTravel, costs, days[0]);
    }

    private int minCostHelper(boolean[] days, int[] costs, int day) {
        ...
        int pathSkip = Integer.MAX_VALUE;
        if (!days[day]) {
            pathSkip = minCostHelper(days, costs, day + 1); // do nothing
        }

        int pathOne = costs[0] + minCostHelper(days, costs, day + 1);
        int pathTwo = costs[1] + minCostHelper(days, costs, day + 7);
        int pathThree = costs[2] + minCostHelper(days, costs, day + 30);

        cache[day] = Math.min(Math.min(Math.min(pathOne, pathTwo), pathThree), pathSkip);
        return cache[day];
    }
}
