package leetcode.editor.en.Q2103;
...
class Solution {
    public int countPoints(String rings) {
        HashMap<Integer, HashSet<Character>> counter = new HashMap<>();
        for (int i = 0; i < rings.length(); i++) {
            char color = rings.charAt(i);
            i++;
            int index = Character.getNumericValue(rings.charAt(i));
            counter.computeIfAbsent(index, (k) -> new HashSet<>()).add(color);
        }
        int res = 0;
        for (HashSet<Character> colors : counter.values()) {
            if (colors.size() == 3) res++;
        }
        return res;
    }
}
...
public class RingsAndRods extends Solution {
}