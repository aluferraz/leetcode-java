package leetcode.editor.en.Q1708;
...
class Solution {
    public int[] largestSubarray(int[] nums, int k) {
        LinkedList<Integer> kWindow = new LinkedList<>();
        for (int i = 0; i < nums.length; i++) {
            kWindow.add(nums[i]);
        }
        int best = 0;
        int i = 0;
        while (kWindow.size() >= k) {
            if (kWindow.poll() > nums[best]) {
                best = i;
            }
            i++;
        }
...
public class LargestSubarrayLengthK extends Solution {
}