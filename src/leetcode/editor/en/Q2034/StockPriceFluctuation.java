package leetcode.editor.en.Q2034;
...
class StockPrice {
    ...
    private final TreeMap<Integer, HashSet<Integer>> stocks;
    private final HashMap<Integer, Integer> stocksMap;
    ...
    public void update(int timestamp, int price) {
        HashSet<Integer> timestampsOfPrice = new HashSet<>();
        if (stocksMap.containsKey(timestamp)) {
            int previousPrice = stocksMap.get(timestamp);
            HashSet<Integer> oldTimestamps = stocks.get(previousPrice);
            oldTimestamps.remove(timestamp);
            if (oldTimestamps.size() == 0) {
                stocks.remove(previousPrice);
            }
        }
        if (this.stocks.containsKey(price)) {
            timestampsOfPrice = stocks.get(price);
        }
        timestampsOfPrice.add(timestamp);
        stocks.put(price, timestampsOfPrice);
        stocksMap.put(timestamp, price);
...
public class StockPriceFluctuation extends StockPrice {
}
